...
export function FamilyAppointmentsPage() {
  useDocumentTitle("مواعيد العلاج");
  ...
  // Appointments for this patient only
  ...
  function handleReschedule() {
    ...
    // Notify both admin and therapist
    addNotification({
      type: "appointment-change",
      title: "طلب إعادة جدولة موعد",
      message: `طلبت أسرة ${patient.fullName} إعادة جدولة موعد ${appt?.type ?? ""} إلى ${formatDate(rescheduleForm.date)} الساعة ${rescheduleForm.time}`,
      targetRole: "doctor",
    });
    addNotification({
      type: "appointment-change",
  ...
  function handleConfirmAttendance(id: string) {
    ...
    showToast("تم تأكيد الحضور للموعد");
  }
  ...
  function handleBook() {
    ...
    addNotification({
      ...
      title: "طلب موعد جديد من الأسرة",
      message: `طلبت أسرة ${patient.fullName} حجز موعد ${bookForm.type} بتاريخ ${formatDate(bookForm.preferredDate)} (${bookForm.channel === "video" ? "عن بُعد" : "حضوري"})`,
      targetRole: "doctor",
    });
    addNotification({
      type: "appointment-request",
  ...
  return (
    <PageContainer maxWidth="max-w-3xl" className="py-8">
      <PageHeader
        title="مواعيد العلاج"
        subtitle={`متابعة مواعيد ${patient.fullName} وتأكيدها بسهولة`}
        actions={
      ...
      {tabbedAppointments.length === 0 ? (
        <Card>
          <EmptyState
            ...
            description={
              activeTab === "upcoming"
                ? `ليس لدى ${patient.fullName} مواعيد مجدولة حالياً. يمكنك طلب موعد جديد بالضغط على زر طلب موعد.`
                : "ستظهر هنا المواعيد السابقة بعد حضورها."
            }
            action={
              activeTab === "upcoming" ? (
                <Button
      ...
      >
        <p className="text-base text-neutral-600 mb-4 leading-relaxed">
          سيتم إرسال طلبك لإعادة الجدولة إلى المركز والمعالج، وسيتم التواصل معك لتأكيد الموعد الجديد.
        </p>
        ...
        <Field label="ملاحظات (اختياري)">
          <Textarea
            ...
            placeholder="أي ملاحظة تودين إضافتها"
          />
        </Field>
      </Modal>
      ...
      >
        <p className="text-base text-neutral-600 mb-4 leading-relaxed">
          املأي البيانات التالية لإرسال طلب حجز موعد لـ{patient.fullName}. سيتم التواصل معك لتأكيد الموعد.
        </p>
