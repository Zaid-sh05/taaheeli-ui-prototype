...
export function AccountStep({ form, update, errors }: Props) {
  return (
    <fieldset className="border-0 p-0 m-0">
      ...
      <Field label="اسم المستخدم" htmlFor="reg-username" required error={errors.username}
        hint="سيستخدم لتسجيل الدخول لاحقاً">
        <Input id="reg-username" value={form.username}
          onChange={(e) => update({ username: e.target.value })}
          hasError={!!errors.username} autoComplete="username" />
      </Field>
      <Field label="كلمة المرور" htmlFor="reg-password" required error={errors.password}
        hint="٨ أحرف على الأقل">
        <Input id="reg-password" type="password" value={form.password}
          onChange={(e) => update({ password: e.target.value })}
          hasError={!!errors.password} autoComplete="new-password" />
      </Field>
      <Field label="تأكيد كلمة المرور" htmlFor="reg-confirm" required error={errors.confirmPassword}>
        <Input id="reg-confirm" type="password" value={form.confirmPassword}
          ...
          hasError={!!errors.confirmPassword} autoComplete="new-password" />
      </Field>
    </fieldset>
  );
}
